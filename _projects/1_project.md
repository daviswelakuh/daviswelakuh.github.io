...
$$
\begin{align}
i\hbar \frac{\partial}{\partial t} \varphi_{i}(\mathbf{r},t) 
&= \left( \frac{\hat{\mathbf{p}}^{2}}{2m} 
+ \underbrace{v(\mathbf{r},t) + v_{\textrm{Mxc}}([n,q_{\alpha}];\mathbf{r},t)}_{v_{\textrm{KS}}([v,n,q_{\alpha}];\mathbf{r},t)} \right)
\varphi_{i}(\mathbf{r},t) 
\\
\left( \frac{\partial^{2}}{\partial t^{2}} + \omega_{\alpha}^{2} \right) q_{\alpha}(t) 
&= \underbrace{-\frac{j_{\alpha}(t)}{\omega_{\alpha}} + \omega_{\alpha}\boldsymbol{\lambda}_{\alpha} \cdot \mathbf{R}(t)}_{j_{\alpha,\textrm{KS}}(t)}
\end{align}
$$
...
= \sum_{k=1}^{N_{e}} \left[ \varphi_{k}^{*}(\textbf{r}) \varphi_{k}^{(+)}(\textbf{r},\omega) +  \varphi_{k}(\textbf{r}) \left[\varphi_{k}^{(-)}(\textbf{r},\omega)\right]^{*}\right] , \quad \textrm{and} \quad
...
$$
\left(\omega - \hat{h} + \epsilon_{k} + i\eta\right)\varphi_{k}^{(+)}(\textbf{r},\omega) =   \delta v_{\textrm{KS}}(\textbf{r},\omega) \varphi_{k}(\textbf{r}) \, ,  \quad \textrm{and} \quad
...
As remark to note is that in the decoupling limit between light and matter (i.e. when $$ \boldsymbol{\lambda}_{\alpha} \rightarrow 0 $$), the Maxwell-Kohn-Sham equations decouples to the electron-only Kohn-Sham equation since $v_{\textrm{Mxc}}(\textbf{r},t) \rightarrow v_{\textrm{Hxc}}(\textbf{r},t)$ and a Maxwell equation for the field. In the electron-photon Casida case, it simplifies to the electron-only Casida equation in the framework of TDDFT given by $$U\, \textbf{E}_{v} = \Omega^{2}_{q} \, \textbf{E}_{v}$$ where $U$ has no dependence on photon-field kernel, i.e., $f_\text{pxc}^{n} \rightarrow 0$. Similary for the electron-photon Sternheimer approach, the photon dispalcement field $q_{\alpha}(\omega)$ decouples and formalismSternheimer equations simplifies to the Sterneheimer equation in the TDDFT framework.

These methods can be used to investigate properties of strongly coupled light-matter systems. Take for example a setting where benzene molecules are placed inside an optical cavity, the confined photon modes of the cavity strongly couple to the electronic excitations of the molecules. In particular, when the frequency of the cavity mode is tuned to be resonant with the $\pi -- \pi^{*}$ electronic transition of benzene, the light-matter interaction enters the strong coupling regime. In this regime, the bare molecular excitation and the cavity photon are no longer independent; instead, they hybridize to form new quasiparticles known as polaritons. This hybridization manifests spectroscopically as a Rabi splitting of the original resonance into two distinct polariton branches in the optical absorption spectrum shown in the figure {% cite welakuh2021thesis %}.
